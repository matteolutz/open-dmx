...
impl DMXSerial {
    ...
    /// ```
    ///
    pub fn open(port: &str, enable_rts: bool) -> Result<DMXSerial, serialport::Error> {
        ...
        let mut agent =
            DMXSerialAgent::open(&port, dmx.min_time_break_to_break.read_only(), enable_rts)?;
    ...
    ///     }
    /// }
    pub fn open_sync(port: &str, enable_rts: bool) -> Result<DMXSerial, serialport::Error> {
        let mut dmx = DMXSerial::open(port, enable_rts)?;
        dmx.set_sync();
        Ok(dmx)
    }
    ...
    pub fn reopen(&mut self) -> Result<(), serialport::Error> {
        ...
        let new_dmx = DMXSerial::open(&self.name, self.enable_rts)?;
        *self = new_dmx;
